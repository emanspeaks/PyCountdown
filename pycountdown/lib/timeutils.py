...
def sec_to_ydhms_str(sec: float, sec_digits: int = 0):
    ymdhms = sec_to_ymdhms(sec, sec_digits)
    ystr = '{:04d}'.format(ymdhms[0])
    doy = day_of_year(*ymdhms[:3])
    dhms = (doy,) + ymdhms[3:]
    return ystr + ':' + dhms_to_met_str(*dhms, sec_digits=sec_digits,
                                        day_digits=3, daysep=':')
